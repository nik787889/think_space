...
export function useYjsStore({
  ...
  const room = useRoom();
  const self = useSelf();
  const userInfo = useMemo(() => self?.info, [self?.info?.id, self?.info?.name, self?.info?.avatar, self?.info?.color]);
  ...
  // Handle Presence syncing in a separate effect to avoid re-syncing the entire document
  useEffect(() => {
    ...
    const roomSelf = room.getSelf();
    // @ts-ignore
    const yClientId = roomSelf?.presence?.__yjs_clientid;
    const presenceId = InstancePresenceRecordType.createId(
      yClientId ? String(yClientId) : "remote"
    );
  ...
  }, [storeWithStatus.status, store, yProvider, userInfo, room]);
